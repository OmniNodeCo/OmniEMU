...
error_handler() {
    ...
    script="""
    set alertTitle to \"OmniEMU - Updater error\"
    set alertMessage to \"An error occurred during OmniEMU update (updater.sh:$lineno)\n\nPlease download the update manually from our website if the problem persists.\"
    ...
    if the button_pressed is \"Open Download Page\" then
        open location \"https://github.com/OmniNodeCo/OmniEMU/releases/latest\"
    end if
    """
