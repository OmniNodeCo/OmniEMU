...
using OmniEMU.HLE.Exceptions;
using OmniEMU.HLE.HOS.Ipc;
using OmniEMU.HLE.HOS.Kernel.Memory;
using OmniEMU.HLE.HOS.Kernel.Threading;
using OmniEMU.HLE.HOS.Services.Am.AppletAE.Storage;
using OmniEMU.HLE.HOS.Services.Am.AppletOE.ApplicationProxyService.ApplicationProxy.Types;
using OmniEMU.HLE.HOS.Services.Sdb.Pdm.QueryService;
using OmniEMU.HLE.HOS.SystemState;
using OmniEMU.Horizon.Common;
using System;
using System.Numerics;
using System.Threading;
using AccountUid = OmniEMU.HLE.HOS.Services.Account.Acc.UserId;
using ApplicationId = LibHac.Ncm.ApplicationId;

namespace OmniEMU.HLE.HOS.Services.Am.AppletOE.ApplicationProxyService.ApplicationProxy
{
    class IApplicationFunctions : IpcService
    {
        ...
        public ResultCode PrepareForJit(ServiceCtx context)
        {
            if (Interlocked.Exchange(ref _jitLoaded, 1) == 0)
            {
                ...
                {
                    throw new InvalidSystemResourceException("JIT (010000000000003B) system title not found! The JIT will not work, provide the system archive to fix this error. (See https://github.com/OmniNodeCo/OmniEMU#requirements for more information)");
                }
