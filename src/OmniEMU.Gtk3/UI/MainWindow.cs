...
using OmniEMU.Audio.Backends.Dummy;
using OmniEMU.Audio.Backends.OpenAL;
using OmniEMU.Audio.Backends.SDL2;
using OmniEMU.Audio.Backends.SoundIo;
using OmniEMU.Audio.Integration;
...
using OmniEMU.Common.SystemInterop;
using OmniEMU.Cpu;
using OmniEMU.Graphics.GAL;
using OmniEMU.Graphics.GAL.Multithreading;
using OmniEMU.HLE.FileSystem;
using OmniEMU.HLE.HOS;
using OmniEMU.HLE.HOS.Services.Account.Acc;
using OmniEMU.HLE.HOS.SystemState;
using OmniEMU.Input.GTK3;
using OmniEMU.Input.HLE;
using OmniEMU.Input.SDL2;
using OmniEMU.Modules;
using OmniEMU.UI.App.Common;
using OmniEMU.UI.Applet;
using OmniEMU.UI.Common;
using OmniEMU.UI.Common.Configuration;
using OmniEMU.UI.Common.Helper;
using OmniEMU.UI.Helper;
using OmniEMU.UI.Widgets;
using OmniEMU.UI.Windows;
...
using ShaderCacheLoadingState = OmniEMU.Graphics.Gpu.Shader.ShaderCacheState;

namespace OmniEMU.UI
{
...
#pragma warning restore CS0649, IDE0044, CS0169, IDE0051
        ...
        public MainWindow() : this(new Builder("OmniEMU.Gtk3.UI.MainWindow.glade")) { }
        ...
        private MainWindow(Builder builder) : base(builder.GetRawOwnedObject("_mainWin"))
        {
            ...
            Icon = new Gdk.Pixbuf(Assembly.GetAssembly(typeof(ConfigurationState)), "OmniEMU.UI.Common.Resources.Logo_OmniEMU.png");
            Title = $"OmniEMU {Program.Version}";
            ...
            VirtualFileSystem.FixExtraData(_libHacHorizonManager.OmniEMUClient);

            _contentManager = new ContentManager(_virtualFileSystem);
            _accountManager = new AccountManager(_libHacHorizonManager.OmniEMUClient, CommandLineState.Profile);
        ...
        private bool LoadApplication(string path, ulong applicationId, bool isFirmwareTitle)
        {
            ...
            if (!SetupValidator.CanStartApplication(_contentManager, path, out UserError userError))
            {
                if (SetupValidator.CanFixStartApplication(_contentManager, path, userError, out firmwareVersion))
                {
                    ...
                    message = $"No installed firmware was found but OmniEMU was able to install firmware {firmwareVersion.VersionString} from the provided game.\nThe emulator will now start.";
            ...
            if (File.Exists(path))
            {
                switch (System.IO.Path.GetExtension(path).ToLowerInvariant())
                {
                    ...
                    default:
                        ...
                        {
                            Logger.Error?.Print(LogClass.Application, "The specified file is not supported by OmniEMU.");
        ...
        private void SwitchToGameTable()
        {
            ...
            Window.Title = $"OmniEMU {Program.Version}";
        ...
        private void Row_Clicked(object sender, ButtonReleaseEventArgs args)
        {
            ...
            _ = new GameTableContextMenu(this, _virtualFileSystem, _accountManager, _libHacHorizonManager.OmniEMUClient, application);
        }
