...
using static OmniEMU.Ava.UI.Helpers.Win32NativeInterop;
using AntiAliasing = OmniEMU.Common.Configuration.AntiAliasing;
using InputManager = OmniEMU.Input.HLE.InputManager;
using IRenderer = OmniEMU.Graphics.GAL.IRenderer;
using Key = OmniEMU.Input.Key;
using MouseButton = OmniEMU.Input.MouseButton;
using ScalingFilter = OmniEMU.Common.Configuration.ScalingFilter;
using Size = Avalonia.Size;
using Switch = OmniEMU.HLE.Switch;

namespace OmniEMU.Ava
{
                ...
                Task.Run(() =>
                {
                    lock (_lockObject)
                    {
                        ...
                        string directory = AppDataManager.Mode switch
                        {
                            ...
                            _ => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "OmniEMU"),
                        };
                        ...
                        if (SetupValidator.CanFixStartApplication(ContentManager, ApplicationPath, userError, out firmwareVersion))
                        {
                            ...
                            if (userError == UserError.NoFirmware)
                            {
                                ...
                                await ContentDialogHelper.CreateInfoDialog(
                                    ...
                                    "",
                                    LocaleManager.Instance[LocaleKeys.OmniEMUInfo]);
                            }
                        }
                        else
                        {
