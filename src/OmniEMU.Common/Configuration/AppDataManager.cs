...
namespace OmniEMU.Common.Configuration
{
    ...
    {
        private const string DefaultBaseDir = "OmniEMU";
        ...
        private static string SetUpLogsDir()
        {
            ...
            else
            {
                ...
                {
                    // NOTE: Should evaluate to "~/Library/Logs/OmniEMU/".
                    ...
                    {
                        // NOTE: Should evaluate to "~/Library/Application Support/OmniEMU/Logs".
                        logDir = Path.Combine(BaseDirPath, "Logs");
                ...
                {
                    // NOTE: Should evaluate to a "Logs" directory in whatever directory OmniEMU was launched from.
                    ...
                    if (string.IsNullOrEmpty(logDir))
                    {
                        // NOTE: Should evaluate to "C:\Users\user\AppData\Roaming\OmniEMU\Logs".
                ...
                {
                    // NOTE: Should evaluate to "~/.config/OmniEMU/Logs".
                    logDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), DefaultBaseDir, "Logs");
            ...
            if (IsPathSymlink(correctApplicationDataDirectoryPath))
            {
                ...
                catch (Exception exception)
                {
                    Logger.Error?.Print(LogClass.Application,
                        $"Critical error copying OmniEMU application data into the temp folder. {exception}");
                    try
                    {
                        ...
                        Logger.Error?.Print(LogClass.Application, $"Please manually move your OmniEMU data from {resolvedPath} to {correctApplicationDataDirectoryPath}, and remove the symlink.");
                    }
                    catch (Exception symlinkException)
                    {
                        Logger.Error?.Print(LogClass.Application, $"Unable to resolve the symlink for OmniEMU application data: {symlinkException}. Follow the symlink at {correctApplicationDataDirectoryPath} and move your data back to the Application Support folder.");
                    }
                    return;
                }
                ...
                {
                    Logger.Error?.Print(LogClass.Application,
                        $"Critical error deleting the OmniEMU application data folder symlink at {correctApplicationDataDirectoryPath}. {exception}");
                    try
                    {
                        FileSystemInfo resolvedDirectoryInfo =
                            Directory.ResolveLinkTarget(correctApplicationDataDirectoryPath, true);
                ...
                {
                    Logger.Error?.Print(LogClass.Application,
                        $"Critical error copying OmniEMU application data into the correct location. {exception}. Please manually move your application data from {tempPath} to {correctApplicationDataDirectoryPath}.");
                }
            }
        }
